...
| 指标 | Monad 主网（官网最新） | 对 Agent Jury 的意义 |
| 吞吐量 | **10,000+ TPS**（设计容量） |每案 8+ 笔上链交易，高频率也不拥堵 |
| 出块时间 | **300ms**（主网实测 ~302ms） |  裁决实时落账，审批不拖慢 Agent 执行 |
| 最终性 | **600ms**（MonadBFT 2 slot） |  宿主拿到的是"已最终确认"的裁决，秒级放行 |
| 交易费用 | **近零** | 免费中间件的成本模型才能成立 |
